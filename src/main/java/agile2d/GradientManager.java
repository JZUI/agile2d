...
class GradientManager {
    ...
    void createTexture() {
        if (texture == 0) {
            ...
            gl.glTexImage1D(GL2.GL_TEXTURE_1D, 0, GL2.GL_RGBA, 2, 0, GL2.GL_RGBA, GL2.GL_FLOAT, null);
        ...
        gl.glTexSubImage1D(GL2.GL_TEXTURE_1D, 0, 0, 2, GL2.GL_RGBA, GL2.GL_FLOAT, buffer);
        gl.glTexParameteri(GL2.GL_TEXTURE_1D, GL2.GL_TEXTURE_WRAP_S, cyclic ? GL2.GL_REPEAT : GL2.GL_CLAMP);
    }
    ...
    public void begin(float alpha) {
        ...
        if (alpha == 1.0f && a1 == 1.0f && a2 == 1.0f) {
            if (glState.setState(GL2.GL_TEXTURE_ENV_MODE, GL2.GL_REPLACE))
                gl.glTexEnvi(GL2.GL_TEXTURE_ENV, GL2.GL_TEXTURE_ENV_MODE, GL2.GL_REPLACE);
        }
        else {
            glState.glColor4f(1, 1, 1, alpha);
            if (glState.setState(GL2.GL_TEXTURE_ENV_MODE, GL2.GL_MODULATE))
    ...
    public void end() {
        if (invalid)
            return;
        glState.glDisable(GL2.GL_TEXTURE_1D);

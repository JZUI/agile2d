 ...
 */
final class Texture {
    private GL2  gl; // Context the texture belongs to
    ...
    public void setPixels(int[] intPixels) {
        ...
        if (pixelFormat != GL2.GL_RGBA|| numComponents != 4) { // for Ints this must be GL2.GL_RGBA
    ...
    void loadPixels(BufferedImage image, Rectangle paintRect) {
        ...
        if (pixelFormat != GL2.GL_RGBA|| numComponents != 4) {
...
//        updateNeeded = true;
        ...
        if (paintRect.x != 0)
            gl.glPixelStorei(GL2.GL_UNPACK_SKIP_PIXELS, paintRect.x);
        if (paintRect.y != 0)
            gl.glPixelStorei(GL2.GL_UNPACK_SKIP_ROWS, paintRect.y);
...
//                                         data, image.getWidth());
    ...
    public synchronized void update() {
        ...
        gl.glPixelStorei(GL2.GL_UNPACK_ROW_LENGTH, textureWidth);
        if (pixelFormat == GL2.GL_RGBA) {
            gl.glPixelStorei(GL2.GL_UNPACK_SWAP_BYTES, 1);
            gl.glPixelStorei(GL2.GL_UNPACK_ALIGNMENT, 4);
        }
        else {
    ...
    private void create() {
        ...
        gl.glTexImage2D(GL2.GL_TEXTURE_2D, 0, internalFormat, textureWidth,
                        textureHeight, 0, pixelFormat, GL2.GL_UNSIGNED_BYTE,
                        null);
    ...
    void enable() {
        ...
        bind(); // Ensures texture is bound to GL2.GL_TEXTURE_2D target
    }
    ...
    public synchronized void paint(double x1, double y1, double x2, double y2) {
        ...
        if (pixelFormat == GL2.GL_RGBA || pixelFormat == GL2.GL_LUMINANCE_ALPHA) {
    ...
    void begin(Rectangle2D anchorRect) {
        ...
        glState.glEnable(GL2.GL_TEXTURE_GEN_T);
        if (pixelFormat == GL2.GL_RGBA || pixelFormat == GL2.GL_LUMINANCE_ALPHA) {
            glState.glEnable(GL2.GL_BLEND);
        }
